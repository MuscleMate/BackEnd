...
const sendRequest = async (req, res) => {
    ...
    try {
        ...
        await sendNotification([userToBeAdded._id], `${user.firstName} sent you a friend request`);
...
const addFriend = async (req, res) => {
    ...
    try {
        ...
        await sendNotification([userToBeAdded._id], `${user.firstName} accepted your friend request`);
        res.status(StatusCodes.NO_CONTENT).json({});
    } catch (error) {
        throw new BadRequestError(error.message);
    }
...
const getChallengesRanking = async (req, res) => {
    ...
    try {
        const user = await User.findById(userID).populate({
            path: "friends",
            select: ["firstName", "lastName", "_id", "stats.challengesCompleted"]
        })
        if (!user) {
            throw new NotFoundError(`User with id ${userID} not found`);
        }
        ...
        const friends = user.friends.map(friend => {
            return { _id: friend._id, firstName: friend.firstName, lastName: friend.lastName, challengesCompleted: friend.stats.challengesCompleted }
        })

        let ranking = [...friends, { _id: userID, firstName: user.firstName, lastName: user.lastName, challengesCompleted: user.stats.challengesCompleted }]
        ranking = ranking.sort((a, b) => b.challengesCompleted - a.challengesCompleted);
        res.status(StatusCodes.OK).json({ ranking });
    } catch (error) {
        throw new BadRequestError(error.message);
    }
...
const getWorkoutsRanking = async (req, res) => {
    ...
    try {
        const user = await User.findById(userID).populate({
            path: "friends",
            select: ["firstName", "lastName", "_id", "stats.workoutsCompleted"]
        })
        if (!user) {
            throw new NotFoundError(`User with id ${userID} not found`);
        }
        ...
        const friends = user.friends.map(friend => {
            return { _id: friend._id, firstName: friend.firstName, lastName: friend.lastName, workoutsCompleted: friend.stats.workoutsCompleted }
        })

        let ranking = [...friends, { _id: userID, firstName: user.firstName, lastName: user.lastName, workoutsCompleted: user.stats.workoutsCompleted }]
        ranking = ranking.sort((a, b) => b.workoutsCompleted - a.workoutsCompleted);
        res.status(StatusCodes.OK).json({ ranking });
    } catch (error) {
        throw new BadRequestError(error.message);
    }
...
const getTournamentsRanking = async (req, res) => {
    ...
    try {
        const user = await User.findById(userID).populate({
            path: "friends",
            select: ["firstName", "lastName", "_id", "stats.tournamentsWon"]
        })
        if (!user) {
            throw new NotFoundError(`User with id ${userID} not found`);
        }
        ...
        const friends = user.friends.map(friend => {
            return { _id: friend._id, firstName: friend.firstName, lastName: friend.lastName, tournamentWins: friend.stats.tournamentsWon }
        })

        let ranking = [...friends, { _id: userID, firstName: user.firstName, lastName: user.lastName, tournamentWins: user.stats.tournamentsWon }]

        ranking = ranking.sort((a, b) => b.tournamentWins - a.tournamentWins);
        res.status(StatusCodes.OK).json({ ranking });
    } catch (error) {
        throw new BadRequestError(error.message);
    }
...
module.exports = {
    getFriends, addFriend, sendRequest, cancelFriend, denyFriend, deleteFriend, getReceivedRequests, getSentRequests,
